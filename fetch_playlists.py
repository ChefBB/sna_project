...
# def patch_spotipy_rate_limit_logging(sp):
#     original_call = sp._internal_call

#     def wrapped_call(method, url, payload=None, params=None, headers=None, retries=3):
#         try:
#             return original_call(method, url, payload, params, headers, retries)
#         except spotipy.exceptions.SpotifyException as e:
#             if e.http_status == 429:
#                 retry_after = int(e.headers.get("Retry-After", 1))
#                 print(f"Rate limited. Sleeping for {retry_after} seconds...")
#                 time.sleep(retry_after)
#                 return original_call(method, url, payload, params, headers, retries - 1)
#             elif e.http_status == 404:
#                 print(f"Resource not found: {e}")
#                 return {}
#             else:
...
#     sp._internal_call = wrapped_call
...
#                 time.sleep(retry_after)
#                 # Retry once with same arguments
#                 return original_call(method, url, payload, params, headers, retries - 1)
#             else:
#                 raise
